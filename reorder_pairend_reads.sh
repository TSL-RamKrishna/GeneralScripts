...
cat $R1 | paste - - - - > reformated_read1.txt ; split --suffix-length 5 --additional-suffix _seqids  -l 100 seqids.txt ;  split --suffix-length 5 --additional-suffix _reads -l 1000 reformated_read1.txt ; rm reformated_read1.txt; for seqid in *_seqid; do for read in *_reads; do grep -w -f $seqid --no-group-separator  $read >> reordered_R1.fastq; done; done
...
cat $R2 | paste - - - - > reformated_read2.txt ; split --suffix-length 5 --additional-suffix _seqids  -l 100 seqids.txt ;  split --suffix-length 5 --additional-suffix _reads -l 1000 reformated_read2.txt ; rm reformated_read2.txt; for seqid in *_seqid; do for read in *_reads; do grep -w -f $seqid --no-group-separator  $read >> reordered_R2.fastq; done; done
